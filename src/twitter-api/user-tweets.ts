...
import { TUserTweetsResponse, TTimelineAddEntriesInstruction, TTweet } from './api-types';
...
export const fetchUserTweets = async ({
  ...
  const addEntriesInstructions = data.data.user.result.timeline_v2.timeline.instructions
    .filter(
  ...
  const filteredTweets = tweets.filter((tweet) =>
    tweet.core.user_results.result.legacy.screen_name.toUpperCase() === username.toUpperCase()
  );
